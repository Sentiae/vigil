...
--
-- Backfill: every finding already open and overdue when this ships has been
-- published by every earlier scan, so it is marked emitted for its current
-- deadline rather than re-emitted as a burst. The runner applies this file and
-- its schema_migrations row in one transaction. No index: the claim is served by
-- idx_findings_sla_deadline (001).
